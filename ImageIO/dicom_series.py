# -*- coding: utf-8 -*-
"""
Created on 10/24/16 12:37 PM

@author: shuang Shih-ying Huang
@goal: modified the dicom_series written by Almar Klein below...
...
def _splitSerieIfRequired(serie, series):
    ...
    # Sort the original list by InstanceNumber
    # and get local name
    serie._sort()
    L = serie._datasets
...
def _splitSerieIfRequired_RJH(serie, series):
    """ _splitSerieIfRequired(serie, series)
    Split the serie in multiple series if this is required.
    The choice is based on examing the image position relative to
    the previous image. If it differs too much, it is assumed
    ...
    for index in range(1, len(L)):
        ...
        # print 'ds1 {}, ds2 {}'.format(ds1.filename, ds2.filename)
        ...
        #np.norm(ds2.ImagePositionPatient-ds1.ImagePositionPatient)
        #[1,0,0,  0,1,0  ?,?,?]
        ...
        # If the distance deviates more than 2x from what we've seen,
        # we can agree it's a new dataset.
        # print 'distance = {}, newDist = {}'.format(distance, newDist)
        if distance and newDist > 2.1 * distance:
            # print 'start a new dataset!'
            L2.append([])
            distance = 0
        else:
            # Test missing file
...
def _getPixelDataFromDataset(ds):
    ...
    if needApplySlopeOffset:
        ...
        # print
        # print 'applied slope and offset: ', ds.RescaleSlope, ' ', ds.RescaleIntercept
...
def read_files(path, showProgress=False, readPixelData=False):
    ...
    for filename in files:
        ...
        # RJH adding filename
        #        series[suid]._append_filename(filename)
    ...
    for serie in reversed([serie for serie in series]):
        #        _splitSerieIfRequired(serie, series)
        _splitSerieIfRequired_RJH(serie, series)
    ...
    for i in range(len(series)):
        #        print '\n------ {} ------'.format(i)
        #        SS = series[i]
        #        tmp_filenames = SS.filenames
        #        if len(tmp_filenames) > 0:
        #            print 'filenames: {}'.format(tmp_filenames[0])
        #        else:
...
class DicomSeries(object):
    ...
    def description(self):
        ...
        #            fields.append("'" + info.SeriesNumber + "'")


        # Combine
        return ' '.join(fields)

    #    #RJH adding filenames
    #    @property
    #    def filenames(self):
    #        """ filenames of dcm files """
    #        return self._filenames

    # RJH adding slice_direction
    @property
    def slice_direction(self):
        """ slice_direction """
        return self._slice_direction

    # RJH adding uniform_orientation
    @property
    def uniform_orientation(self):
        """ If all dcms in series have the same
        orientation, then True, otherwise False """
        return self._uniform_orientation
    ...
    #    #RJH adding function to append filename to list of filenames
    #    def _append_filename(self,filename):
    ...
    def _finish(self):
        ...
        if len(L) == 0:
            ...
            self._uniform_orientation = True  # RJH
            return
        ...
        for index in range(len(L)):
            ...
            # RJH adding check for uniform orientation
        ...
        if len(L) > 1:
            ...
            ds2 = L[-1]
            for idx in range(3):
                ddd[idx] = (float(ds2.ImagePositionPatient[idx]) - float(ds1.ImagePositionPatient[idx])) / (len(L) - 1)

                # normalize slice_direction array
            V = np.array(ddd)
